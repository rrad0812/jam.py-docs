...
- Metoda je označena kao `@staticmethod` jer ne zavisi od stanja instance
- Može se koristiti kao pomoćna funkcija izvan konteksta instance (npr. u `server_classes.py`)
...
- **False → None konverzija:** Ako je `field.data` Python `False` ali tip polja nie `consts.BOOLEAN`, konvertuje se u `None` (SQL NULL)
